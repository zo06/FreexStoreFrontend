...
export default function Dashboard() {
  ...
  const handleStartFreeTrial = async () => {
    try {
      ...
      if (result.success) {
        toast.success('Free trial started! You now have 3 days to try up to 2 scripts.')
        // Reload the page to refresh user data
        window.location.reload()
      }
    } catch (error: any) {
      console.error('Failed to start free trial:', error)
      toast.error(error?.message || 'Failed to start free trial. Please try again.')
    } finally {
      ...
      <div className="container px-4 py-4 mx-auto lg:px-6 lg:py-8">
        ...
        <div className="mb-8 lg:mb-12 animate-fade-in">
          <div className="flex flex-col gap-4 justify-between mb-4 lg:flex-row lg:items-center lg:mb-6">
            <div>
              <h1 className="mb-2 text-2xl font-bold lg:text-4xl xl:text-5xl text-gradient">Welcome back, {user?.username || 'User'}</h1>
          ...
          <div className="space-y-8 animate-fade-in">
              ...
              <div className="mb-8 p-6 rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-cyan-900/40 via-slate-900/80 to-blue-900/40 backdrop-blur-xl animate-fade-in">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-white mb-1">Start Your Free Trial</h3>
                      <p className="text-sm text-slate-400">
                        Get <span className="text-cyan-400 font-semibold">3 days</span> of free access to try up to <span className="text-cyan-400 font-semibold">2 scripts</span>. No credit card required!
                      </p>
                    </div>
                  </div>
                  <div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-8">
              ...
              <div className="p-4 card-modern lg:p-6">
                <h3 className="mb-4 text-lg font-semibold text-white lg:text-xl lg:mb-6">Recent Activity</h3>
              ...
              {/* Recent Payments */}
              <div className="p-4 card-modern lg:p-6">
                <h3 className="mb-4 text-lg font-semibold text-white lg:text-xl lg:mb-6">Recent Payments</h3>
                <div className="space-y-3 lg:space-y-4">
                  {dataLoading ? (
                    <div className="flex justify-center items-center py-8">
                      <div className="w-8 h-8 rounded-full border-b-2 border-cyan-400 animate-spin"></div>
                    ...
                    recentPurchases.slice(0, 4).map((purchase) => {
                      ...
                      <div key={purchase.id} className="flex gap-3 items-center p-3 rounded-lg transition-colors cursor-pointer lg:gap-4 lg:p-4 bg-white/5 hover:bg-white/10">
                        ...
                        <div className="text-right">
                          <p className="text-sm font-bold text-gradient">{purchase.price === 0 ? 'Free Trial' : `$${purchase.price}`}</p>
                          <span className={`text-xs px-2 py-0.5 rounded-full ${purchase.status === 'completed' ? 'bg-green-500/20 text-green-400' : purchase.status === 'refunded' ? 'bg-blue-500/20 text-blue-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                            {purchase.status === 'completed' ? 'Completed' : purchase.status === 'refunded' ? 'Refunded' : 'Pending'}
                          </span>
                        </div>
                      </div>
                    )})
          ...
          <div className="animate-fade-in">
            <div className="p-4 card-modern lg:p-6">
              <div className="flex flex-col gap-3 justify-between mb-4 sm:flex-row sm:items-center lg:mb-6">
                <div>
                  <h3 className="text-lg font-semibold text-white lg:text-xl">My Scripts</h3>
                  <p className="text-sm text-muted">Manage your purchased scripts and licenses</p>
                </div>
                      ...
                      <div className="flex flex-col gap-4 justify-between lg:flex-row lg:items-start">
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-1 gap-4 mb-4 lg:grid-cols-2">
                            <div>
                              ...
                              <div className="flex gap-2 items-center">
                                <input 
                                  ...
                                  value={showPrivateKeys.has(license.id) ? (license.privateKey || 'No private key') : '••••••••••••••••••••••••••••••••'}
                                  readOnly
                            ...
                            <div>
                               <label className="block mb-1 text-xs font-medium text-muted">Allowed IP</label>
                          ...
                          <div className="flex flex-wrap gap-4 items-center text-xs text-muted">
                            <span>License Type: <span className="font-medium text-white">
                              {license.expiresAt ? 'Time-based' : 'Lifetime'}
                            </span></span>
                            {license.expiresAt && (
                            ...
                            <span>Last IP: <span className="font-mono font-medium text-white">{license.lastUsedIp || 'Never used'}</span></span>
                          </div>
                        </div>
                        
                        <div className="flex flex-col gap-2">
                          <span className="text-sm font-bold text-accent lg:text-base">{license.isTrial ? 'Free Trial' : `$${license.script?.price || 0}`}</span>
                          <div className="flex flex-col gap-2">
                            <Button 
                              variant="outline" 
                              size="sm" 
              ...
              {!licenses?.data || licenses.data.length === 0 && (
                <div className="py-8 text-center">
                  ...
                  <h4 className="mb-2 font-semibold text-white">No Scripts Found</h4>
                  <p className="mb-4 text-sm text-muted">You haven't purchased any scripts yet.</p>
                  <Button 
          ...
          <div className="p-6 mx-4 w-full max-w-md bg-gray-900 rounded-lg border border-cyan-500/30">
            ...
            <div className="mb-6">
              ...
              <p className="mb-4 text-sm text-gray-300">
                Action: <span className="font-semibold text-cyan-400">{selectedScript.action}</span>
              </p>
