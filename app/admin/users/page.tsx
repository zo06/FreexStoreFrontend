...
function AdminUsers() {
  ...
  const filterConfig: FilterConfig = {
    searchPlaceholder: "Search by username or Discord name...",
    statusOptions: [
    ...
    roleOptions: [
      { value: 'admin', label: 'Admin', count: users.filter(u => u.isAdmin).length },
      { value: 'user', label: 'User', count: users.filter(u => !u.isAdmin).length }
    ],
    showDateFilter: true,
  ...
  const getStatusBadge = (user: User) => {
    ...
    if (user.isAdmin) {
      return <Badge variant="default" className="bg-cyan-500">Admin</Badge>
    }
        ...
        <div className="p-6 rounded-2xl border shadow-2xl backdrop-blur-xl bg-white/5 border-white/10">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-300">User Management</h1>
                <p className="mt-1 text-gray-400">Manage all platform users and their permissions</p>
              </div>
            </div>
            <Button 
      ...
      />
        ...
        <div className="p-6 rounded-2xl border shadow-2xl backdrop-blur-xl transition-all duration-300 bg-white/5 border-white/10 hover:bg-white/10">
          <div className="mb-6">
            ...
            <p className="mt-1 text-gray-400">List of all platform users</p>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/10 hover:bg-white/5">
                  ...
                  <TableHead className="font-semibold text-gray-300">Discord</TableHead>
              ...
              <TableBody>
                {filteredUsers.map((user) => (
                  <TableRow key={user.id} className="transition-colors border-white/10 hover:bg-white/5">
                    ...
                    <TableCell className="text-gray-300">{user.discordUsername || 'N/A'}</TableCell>
        ...
        <div className="p-4 rounded-2xl border shadow-2xl backdrop-blur-xl bg-white/5 border-white/10">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-400">
              Showing {Math.min((currentPage - 1) * limit + 1, users.length)}-{Math.min(currentPage * limit, users.length)} of {users.length} users
            </p>
            <div className="flex space-x-2">
              <Button
